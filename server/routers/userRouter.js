...
router.get('/:id', (req, res) => {
    queries.getOne('users', req.params.id).then(sticker => {
...
router.get('/', (req, res) => {
    queries.getAll('users').then(response => {
        res.json(response);
    })
})
...
router.post('/', (req, res, next) => {
    //insert into db
    queries.create('users', req.body).then(response => {
        res.json(response);
    })
})
...
router.put('/:id', (req, res, next) => {
    // update the db
    queries.update('users', req.params.id, req.body).then(stickers => {
...
router.delete('/:id', (req, res, next) => {
    queries.delete('users', req.params.id).then(() => {
        res.json({
            deleted: true
        })
    })
