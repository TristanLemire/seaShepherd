...
router.get('/:id', (req, res) => {
    queries.getOne('admins', req.params.id).then(sticker => {
...
router.get('/', (req, res) => {
    queries.getAll('admins').then(response => {
        res.json(response);
    })
})
...
router.post('/', (req, res, next) => {
    //insert into db
    queries.create('admins', req.body).then(response => {
        res.json(response);
    })
})
...
router.put('/:id', (req, res, next) => {
    // update the db
    queries.update('admins', req.params.id, req.body).then(stickers => {
...
router.delete('/:id', (req, res, next) => {
    queries.delete('admins', req.params.id).then(() => {
        res.json({
            deleted: true
        })
    })
